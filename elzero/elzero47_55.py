...
else:                                                       # we use else with while to show what happen when it finish

  print("Bookmark Is Full, You Cant Add More")
# Check If List Is Not Empty                                # look at to know how we can use if also for while loop
if len(myFavouriteWebs) > 0:                                # make while loop on if sentence
  # Sort The List
...
for name in peoples:                                        # NOTE: how loop for dict
...
for skill in mySkills:                                  # 1st : key   dict[key]

  print(f"{skill} => {mySkills[skill]}")

#######################

for skill_key, skill_progress in mySkills.items():      # 2nd : key,value in dict.items() == is key and value in our dict items
...
for main_key, main_value in myUltimateSkills.items():     # how deal with nesting dict

  print(f"{main_key} Progress Is: ")                      # 2nd for loop deal with first value, is them on first value?
